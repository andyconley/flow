"""Two guarded MAF participants for Flow's mixed-provider execution protocol.
    ...
    class CodexBuild(Executor):
        ...
        async def accepted(self, request: dict[str, Any], response: dict[str, Any], ctx: WorkflowContext[Never, dict[str, str]]) -> None:
            if request != {"kind": "delegate", "sequence": 2} or response.get("status") != "completed":
                raise RuntimeError("Codex specialist did not complete")
            await ctx.yield_output({"reason": "mixed-job-complete"})

    local, codex = LocalPlan(id="flow-local-test-engineer"), CodexBuild(id="flow-codex-lead-developer")
    return WorkflowBuilder(start_executor=local, name="flow-maf-mixed-provider-v3").add_chain([local, codex]).build()
...
async def _run(start: dict[str, Any]) -> None:
    ...
    if not isinstance(envelope, dict) or envelope.get("execution_protocol_version") != 3:
        raise RuntimeError("mixed child requires a v3 Flow envelope")
