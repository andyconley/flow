...
def main() -> None:
    ...
    assert gate.decide(request(1)).allowed
    assert gate.claim_dispatch("r-1")
    gate.mark_unknown("r-1", "stream_observed_process_exit")
    assert gate.request_status("r-1") == "unknown"
    gate.require_reconciliation("r-1")
    gate.require_reconciliation("r-1")
    assert sum(event[2] == "manual_reconciliation_required" for event in gate.snapshot()["events"]) == 1
    assert not gate.decide(request(1)).allowed
    assert not gate.claim_dispatch("r-1")
    assert gate.decide(request(2)).allowed
    assert gate.decide(request(3)).allowed
