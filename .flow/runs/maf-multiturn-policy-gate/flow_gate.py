...
PROVIDERS = {
    ...
    "local-stub": {"paid": False, "hard_cap": 0.0, "dispatchable": True},
...
class FlowGate:
    ...
    def decide(self, request: dict[str, object]) -> Decision:
        ...
        with self._connect() as db:
            ...
            elif kind == "delegate" and self._count(db, "SELECT count(*) FROM requests WHERE kind='delegate' AND status IN ('allowed','completed','unknown')") >= MAX_DELEGATIONS:
                reason = "delegation_cap"
            elif kind == "delegate" and self._count(db, "SELECT count(*) FROM requests WHERE kind='delegate' AND status IN ('allowed','unknown')") >= MAX_CONCURRENT:
                reason = "concurrency_cap"
            elif kind == "replan" and self._count(db, "SELECT count(*) FROM requests WHERE kind='replan' AND status IN ('allowed','completed','unknown')") >= MAX_REPLANS:
                reason = "replan_cap"
            ...
            elif provider_info["paid"] and float(db.execute("SELECT coalesce(sum(hard_cap_usd),0) FROM requests WHERE status IN ('allowed','completed','unknown')").fetchone()[0]) + float(cap) > PAID_BUDGET_USD:
    ...
    def mark_unknown(self, request_id: str, reason: str) -> None:
        if reason not in {"stream_observed_process_exit", "process_died_without_completion"}:
            raise ValueError("unknown-state reason is not recognized")
        with self._connect() as db:
            db.execute("BEGIN IMMEDIATE")
            row = db.execute("SELECT state FROM dispatches WHERE request_id=?", (request_id,)).fetchone()
            if row == ("unknown",):
            ...
            if row != ("started",):
                raise ValueError("dispatch not active")
            db.execute("UPDATE dispatches SET state='unknown' WHERE request_id=?", (request_id,))
            db.execute("UPDATE requests SET status='unknown' WHERE request_id=?", (request_id,))
            db.execute("INSERT INTO events(request_id,event,detail) VALUES(?,?,?)", (request_id, "worker_unknown", reason))
    ...
    def require_reconciliation(self, request_id: str) -> None:
        with self._connect() as db:
            db.execute("BEGIN IMMEDIATE")
            row = db.execute("SELECT status FROM requests WHERE request_id=?", (request_id,)).fetchone()
            if row != ("unknown",):
                raise ValueError("request is not unknown")
            if not db.execute("SELECT 1 FROM events WHERE request_id=? AND event='manual_reconciliation_required'", (request_id,)).fetchone():
                db.execute("INSERT INTO events(request_id,event,detail) VALUES(?,?,?)", (request_id, "manual_reconciliation_required", "do_not_retry"))
