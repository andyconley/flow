...
- **Closing happens just before the seal.** Open expansions are closed in their own transaction just before the receipt is sealed. If the seal then fails, the attempt stays started with its grant lapsed, and a lead supersede recovers it.
- **A hard denial's restore position is best effort.** If binding it fails, the denial still stands, and a later manager pause refuses to resume instead.
